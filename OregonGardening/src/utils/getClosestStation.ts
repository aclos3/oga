...
// returns the closest point in the list to the origin
export function getClosestPoint(origin: Coordinates, points: Coordinates[]): Coordinates | null {
  if (points.length === 0) return null;
  let closest = points[0];
  let minDist = getDistanceFromLatLongInKm(origin, closest);
  for (let i = 1; i < points.length; i++) {
    const dist = getDistanceFromLatLongInKm(origin, points[i]);
    if (dist < minDist) {
      minDist = dist;
      closest = points[i];
    }
