import { Coordinates, getClosestPoint, getDistanceFromLatLongInKm } from './getClosestStation';
...
describe('getClosestStation', () => {
  test('Distance to closest weather station <= 20 KM', () => {
    for (const location of locations) {
      const closest: Coordinates | null = getClosestPoint(location, locations);
      expect(closest).not.toBeNull();

      if (closest !== null) {
        const distance = getDistanceFromLatLongInKm(location, {lat: closest.lat, long: closest.long});
  ...
  test('Returns null if input location is invalid', () => {
    ...
    const closest: Coordinates | null = getClosestPoint(origin, locations);
    expect(closest).not.toBeNull();
  });
});
